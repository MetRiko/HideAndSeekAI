...
var _version = __nccwpck_require__(235);
...
const generatorFunction = (code, details = {}) => {
    const message = messages_messages[code];
    if (!message) {
...
const logger_logger = (process.env.NODE_ENV === 'production' ? null : (() => {
...
var workbox_routing_version = __nccwpck_require__(707);
    ...
    constructor(regExp, handler, method) {
        ...
        const match = ({ url }) => {
            ...
            if ((url.origin !== location.origin) && (result.index !== 0)) {
                if (process.env.NODE_ENV !== 'production') {
                    logger_logger.debug(`The regular expression '${regExp}' only partially matched ` +
...
class Router {
    ...
    addCacheListener() {
        ...
        self.addEventListener('message', ((event) => {
            if (event.data && event.data.type === 'CACHE_URLS') {
                ...
                if (process.env.NODE_ENV !== 'production') {
                    logger_logger.debug(`Caching URLs from the window`, payload.urlsToCache);
                }
    ...
    handleRequest({ request, event }) {
        ...
        if (!url.protocol.startsWith('http')) {
            if (process.env.NODE_ENV !== 'production') {
                logger_logger.debug(`Workbox Router only supports URLs that start with 'http'.`);
            }
            return;
        }
        ...
        if (!handler) {
            if (process.env.NODE_ENV !== 'production') {
                ...
                logger_logger.debug(`No route found for: ${getFriendlyURL(url)}`);
            }
            return;
        }
        if (process.env.NODE_ENV !== 'production') {
            ...
            logger_logger.groupCollapsed(`Router is responding to: ${getFriendlyURL(url)}`);
            debugMessages.forEach((msg) => {
                if (Array.isArray(msg)) {
                    logger_logger.log(...msg);
                }
                else {
                    logger_logger.log(msg);
                }
            ...
            responsePromise = responsePromise.catch(async (err) => {
                ...
                if (catchHandler) {
                    if (process.env.NODE_ENV !== 'production') {
                        ...
                        logger_logger.groupCollapsed(`Error thrown when responding to: ` +
                            ` ${getFriendlyURL(url)}. Falling back to route's Catch Handler.`);
                        logger_logger.error(`Error thrown by:`, route);
            ...
            if (matchResult) {
                if (process.env.NODE_ENV !== 'production') {
                    ...
                    if (matchResult instanceof Promise) {
                        logger_logger.warn(`While routing ${getFriendlyURL(url)}, an async ` +
    ...
    if (typeof capture === 'string') {
        ...
        if (process.env.NODE_ENV !== 'production') {
            if (!(capture.startsWith('/') || capture.startsWith('http'))) {
                throw new WorkboxError_WorkboxError('invalid-string', {
                    moduleName: 'workbox-routing',
                    funcName: 'registerRoute',
                    paramName: 'capture',
                });
            ...
            if ((new RegExp(`${wildcards}`)).exec(valueToCheck)) {
                logger_logger.debug(`The '$capture' parameter contains an Express-style wildcard ` +
        ...
        const matchCallback = ({ url }) => {
            if (process.env.NODE_ENV !== 'production') {
                if ((url.pathname === captureUrl.pathname) &&
                    ...
                    logger_logger.debug(`${capture} only partially matches the cross-origin URL ` +
...
var workbox_precaching_version = __nccwpck_require__(486);
...
const logGroup = (groupTitle, deletedURLs) => {
    logger_logger.groupCollapsed(groupTitle);
    for (const url of deletedURLs) {
        logger_logger.log(url);
    }
    logger_logger.groupEnd();
};
/**
...
function printCleanupDetails(deletedURLs) {
    ...
    if (deletionCount > 0) {
        logger_logger.groupCollapsed(`During precaching cleanup, ` +
...
function printInstallDetails(urlsToPrecache, urlsAlreadyPrecached) {
    ...
    if (precachedCount || alreadyPrecachedCount) {
        ...
        logger_logger.groupCollapsed(message);
...
async function executeQuotaErrorCallbacks() {
    if (process.env.NODE_ENV !== 'production') {
        logger_logger.log(`About to run ${quotaErrorCallbacks_quotaErrorCallbacks.size} ` +
            `callbacks to clean up caches.`);
    }
    for (const callback of quotaErrorCallbacks_quotaErrorCallbacks) {
        await callback();
        if (process.env.NODE_ENV !== 'production') {
            logger_logger.log(callback, 'is complete.');
        }
    }
    if (process.env.NODE_ENV !== 'production') {
        logger_logger.log('Finished running callbacks.');
    }
}
...
var workbox_strategies_version = __nccwpck_require__(534);
...
class StrategyHandler {
    ...
    async fetch(input) {
        ...
        if (request.mode === 'navigate' &&
            ...
            if (possiblePreloadResponse) {
                if (process.env.NODE_ENV !== 'production') {
                    logger_logger.log(`Using a preloaded navigation response for ` +
        ...
        try {
            ...
            if (process.env.NODE_ENV !== 'production') {
                logger_logger.debug(`Network request for ` +
            ...
            if (process.env.NODE_ENV !== 'production') {
                logger_logger.log(`Network request for ` +
    ...
    async cacheMatch(key) {
        ...
        if (process.env.NODE_ENV !== 'production') {
            if (cachedResponse) {
                logger_logger.debug(`Found a cached response in '${cacheName}'.`);
            }
            else {
                logger_logger.debug(`No cached response found in '${cacheName}'.`);
            }
        }
    ...
    async cachePut(key, response) {
        ...
        if (!response) {
            if (process.env.NODE_ENV !== 'production') {
                logger_logger.error(`Cannot cache non-existent response for ` +
        ...
        if (!responseToCache) {
            if (process.env.NODE_ENV !== 'production') {
                logger_logger.debug(`Response '${getFriendlyURL(effectiveRequest.url)}' ` +
        ...
        cache, effectiveRequest.clone(), ['__WB_REVISION__'], matchOptions) :
            null;
        if (process.env.NODE_ENV !== 'production') {
            logger_logger.debug(`Updating the '${cacheName}' cache with a new Response ` +
                ...
                if (responseToCache) {
                    if (responseToCache.status !== 200) {
                        if (responseToCache.status === 0) {
                            logger_logger.warn(`The response for '${this.request.url}' ` +
                                `is an opaque response. The caching strategy that you're ` +
                                `using will not cache opaque responses by default.`);
                        }
                        else {
                            logger_logger.debug(`The response for '${this.request.url}' ` +
    ...
    async _getResponse(handler, request, event) {
        ...
        try {
            ...
            if (!response) {
                ...
                logger_logger.log(`While responding to '${getFriendlyURL(request.url)}', ` +
...
class PrecacheStrategy extends Strategy_Strategy {
    ...
    async _handleFetch(request, handler) {
        ...
        if (this._fallbackToNetwork) {
            if (process.env.NODE_ENV !== 'production') {
                logger_logger.warn(`The precached response for ` +
        ...
        if (process.env.NODE_ENV !== 'production') {
            ...
            // print the routing details to the console.
            logger_logger.groupCollapsed(`Precaching is responding to: ` +
                getFriendlyURL(request.url));
            logger_logger.log(`Serving the precached url: ${getFriendlyURL(cacheKey.url)}`);
        ...
        for (const entry of entries) {
            ...
            if (urlsToWarnAbout.length > 0) {
                ...
                if (process.env.NODE_ENV === 'production') {
                    ...
                    logger_logger.warn(warningMessage);
                }
            }
        }
    }
...
class PrecacheRoute extends Route_Route {
    ...
    constructor(precacheController, options) {
        const match = ({ request }) => {
            ...
            if (process.env.NODE_ENV !== 'production') {
                logger_logger.debug(`Precaching did not find a match for ` +
                    getFriendlyURL(request.url));
            }
            return;
        };
...
;// CONCATENATED MODULE: ./node_modules/preact-cli/sw/utils.js
...
class NetworkFirst extends Strategy_Strategy {
    ...
    async _handle(request, handler) {
        ...
        if (process.env.NODE_ENV !== 'production') {
            logger_logger.groupCollapsed(utils_messages_messages.strategyStart(this.constructor.name, request));
            for (const log of logs) {
                logger_logger.log(log);
            }
            utils_messages_messages.printFinalResponse(response);
            logger_logger.groupEnd();
        }
        if (!response) {
            throw new WorkboxError_WorkboxError('no-response', { url: request.url });
...
var workbox_cacheable_response_version = __nccwpck_require__(987);
...
class CacheableResponse {
    ...
    constructor(config = {}) {
        if (process.env.NODE_ENV !== 'production') {
            if (!(config.statuses || config.headers)) {
                throw new WorkboxError_WorkboxError('statuses-or-headers-required', {
    ...
    isResponseCacheable(response) {
        ...
        if (process.env.NODE_ENV !== 'production') {
            if (!cacheable) {
                logger_logger.groupCollapsed(`The request for ` +
                    `'${getFriendlyURL(response.url)}' returned a response that does ` +
                    `not meet the criteria for being cached.`);
                logger_logger.groupCollapsed(`View cacheability criteria here.`);
                logger_logger.log(`Cacheable statuses: ` +
                    JSON.stringify(this._statuses));
                logger_logger.log(`Cacheable headers: ` +
                ...
                logger_logger.groupCollapsed(`View response status and headers here.`);
                logger_logger.log(`Response status: ` + response.status);
                logger_logger.log(`Response headers: ` +
                    JSON.stringify(logFriendlyHeaders, null, 2));
                logger_logger.groupEnd();
                logger_logger.groupCollapsed(`View full response details here.`);
                logger_logger.log(response.headers);
                logger_logger.log(response);
                logger_logger.groupEnd();
                logger_logger.groupEnd();
...
;// CONCATENATED MODULE: ./node_modules/preact-cli/sw/contants.js




const PRECACHING_OPTIONS = {};
...
const NETWORK_HANDLER = new NetworkFirst({
	...
	cacheName: workbox_core_cacheNames_cacheNames.precache,
...
;// CONCATENATED MODULE: ./node_modules/preact-cli/sw/index.js
...
function setupPrecaching(precacheFiles, precachingOptions) {
